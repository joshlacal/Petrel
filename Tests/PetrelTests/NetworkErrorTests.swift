...
struct NetworkErrorTests {
    ...
    func networkErrorDescriptions() {
        ...
        let streamOverflowError = NetworkError.streamOverflow
        #expect(streamOverflowError.localizedDescription.contains("stream buffer exceeded capacity"))

        let responseLimitError = NetworkError.responseLimitExceeded("size exceeded")
        #expect(responseLimitError.localizedDescription.contains("security limits"))

        let metadataError = NetworkError.metadataStrippingFailed
        #expect(metadataError.localizedDescription.contains("metadata"))
    }
