...
- **Capstone e2e**: `atproto/packages/dev-env` (real local PDS + AS); a small demo CLI completes an actual login — PAR with assertion → authorize → token exchange → refresh → authenticated XRPC call. Done-criteria per workspace CLAUDE.md: observable end-to-end behavior, not just green tests.
