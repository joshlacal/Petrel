...
- **Rate limiting and deny list**: The server rate-limits per JKT (DPoP public key hash) and supports a deny list for compromised keys. This bounds abuse from a single, well-behaved client key and lets you cut off a known-compromised one — it does **not** stop an attacker who mints a fresh DPoP key per request, since each key gets its own budget. That residual risk is denial-of-service only: minted assertions are DPoP-bound (`cnf`/`jkt`), so a flooder gains no usable credentials, only wasted server cycles. Mitigate at the network/proxy layer (e.g. a reverse-proxy or WAF rate limit ahead of this server) if that's a concern for your deployment.
