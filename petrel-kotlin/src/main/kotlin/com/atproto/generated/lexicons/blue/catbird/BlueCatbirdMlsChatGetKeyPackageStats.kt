...
object BlueCatbirdMlsChatGetKeyPackageStatsDefs {
    const val TYPE_IDENTIFIER = "blue.catbird.mlsChat.getKeyPackageStats"
}
     ...
     */
    @Serializable
    data class BlueCatbirdMlsChatGetKeyPackageStatsCipherSuiteStats(
/** Cipher suite name */        @SerialName("cipherSuite")
        ...
        val consumed: Int?    ) {
        companion object {
            const val TYPE_IDENTIFIER = "#blueCatbirdMlsChatGetKeyPackageStatsCipherSuiteStats"
        }
    }

@Serializable
    data class BlueCatbirdMlsChatGetKeyPackageStatsParameters(
// DID to fetch stats for (defaults to authenticated user)        @SerialName("did")
    ...
    @Serializable
    data class BlueCatbirdMlsChatGetKeyPackageStatsOutput(
// Number of unconsumed key packages available        @SerialName("available")
        ...
        val byCipherSuite: List<BlueCatbirdMlsChatGetKeyPackageStatsCipherSuiteStats>? = null    )

sealed class BlueCatbirdMlsChatGetKeyPackageStatsError(val name: String, val description: String?) {
        object InvalidDid: BlueCatbirdMlsChatGetKeyPackageStatsError("InvalidDid", "The provided DID is invalid")
    }

/**
 * Get key package inventory statistics for the authenticated user to determine when replenishment is needed
 *
 * Endpoint: blue.catbird.mlsChat.getKeyPackageStats
 */
suspend fun ATProtoClient.Blue.Catbird.MlsChat.getKeyPackageStats(
parameters: BlueCatbirdMlsChatGetKeyPackageStatsParameters): ATProtoResponse<BlueCatbirdMlsChatGetKeyPackageStatsOutput> {
    val endpoint = "blue.catbird.mlsChat.getKeyPackageStats"
