...
object BlueCatbirdMlsChatGetReportsDefs {
    const val TYPE_IDENTIFIER = "blue.catbird.mlsChat.getReports"
}
     ...
     */
    @Serializable
    data class BlueCatbirdMlsChatGetReportsReportView(
/** Report identifier */        @SerialName("id")
        ...
        companion object {
            const val TYPE_IDENTIFIER = "#blueCatbirdMlsChatGetReportsReportView"
        }
    }

@Serializable
    data class BlueCatbirdMlsChatGetReportsParameters(
// Conversation identifier        @SerialName("convoId")
    ...
    @Serializable
    data class BlueCatbirdMlsChatGetReportsOutput(
// List of reports (encrypted content)        @SerialName("reports")
        val reports: List<BlueCatbirdMlsChatGetReportsReportView>    )

sealed class BlueCatbirdMlsChatGetReportsError(val name: String, val description: String?) {
        object NotAdmin: BlueCatbirdMlsChatGetReportsError("NotAdmin", "Caller is not an admin")
        object ConvoNotFound: BlueCatbirdMlsChatGetReportsError("ConvoNotFound", "Conversation not found")
    }

/**
 * Get reports for a conversation (admin-only) Retrieve reports for a conversation. Admin-only endpoint. Returns encrypted report blobs that admins must decrypt locally using MLS group key.
 *
 * Endpoint: blue.catbird.mlsChat.getReports
 */
suspend fun ATProtoClient.Blue.Catbird.MlsChat.getReports(
parameters: BlueCatbirdMlsChatGetReportsParameters): ATProtoResponse<BlueCatbirdMlsChatGetReportsOutput> {
    val endpoint = "blue.catbird.mlsChat.getReports"
