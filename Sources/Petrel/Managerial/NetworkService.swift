...
public actor NetworkService: NetworkServiceProtocol {
  ...
  internal func setProtectedResourceMetadata(_ metadata: ProtectedResourceMetadata) {
    self.protectedResourceMetadata = metadata
    LogManager.logInfo("Network Service - Protected Resource Metadata updated")
  }

  internal func setAuthorizationServerMetadata(_ metadata: AuthorizationServerMetadata) {
  ...
  public func setUserAgent(_ userAgent: String) async {
    ...
    await self.setHeader(name: "User-Agent", value: userAgent)
  }
  ...
  public func request(_ request: URLRequest, skipTokenRefresh: Bool = false) async throws -> (
    ...
    while retryCount < maxRetries {
      ...
      if requiresAuth, let authProvider = self.authProvider {
        do {
      ...
      do {
        ...
        if let authProvider = self.authProvider, let url = requestToSend.url {
        ...
        case 401:
          ...
          guard let authProvider = self.authProvider, let url = requestToSend.url else {
            LogManager.logError(
