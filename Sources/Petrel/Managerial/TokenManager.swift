...
public actor TokenManager: TokenManaging {
    ...
    private func loadDPoPBindings() async {
        LogManager.logDebug(">>> TokenManager.loadDPoPBindings STARTING")
        do {
            let namespace = await getNamespace()
            LogManager.logDebug("TokenManager - Attempting to load DPoP bindings for namespace: \(namespace)")
        ...
        LogManager.logDebug("<<< TokenManager.loadDPoPBindings FINISHED")
    }

    private func saveDPoPBindings() async throws {
        let data = try JSONEncoder().encode(dpopKeyBindings)
        try KeychainManager.store(key: dpopBindingsKey, value: data, namespace: await getNamespace())
        LogManager.logDebug("TokenManager - Saved DPoP bindings to Keychain")
    }

    // Update init to take AccountManager
    init(accountManager: AccountManaging) async {
        LogManager.logDebug(">>> TokenManager.init STARTING")
        self.accountManager = accountManager
        await loadInitialTokens()
        await loadDPoPBindings()
        await subscribeToEvents()
        LogManager.logDebug("<<< TokenManager.init FINISHED")
        LogManager.logDebug("Token Manager initialized") // Original log kept for context
    }
        ...
        Task {
            for await event in eventStream {
                ...
                case .activeAccountChanged:
                    // Reload tokens when active account changes
                    await loadInitialTokens()
                    await loadDPoPBindings()
                default:
                    break
    ...
    private func loadInitialTokens() async {
        LogManager.logDebug(">>> TokenManager.loadInitialTokens STARTING")
        let namespace = await getNamespace()
        LogManager.logDebug("TokenManager - Attempting to load tokens for namespace: \(namespace)")
        ...
        {
            tokenType = type
            LogManager.logDebug("TokenManager - Loaded token type: \(type.rawValue)")
        } else {
            LogManager.logDebug("TokenManager - No token type found in Keychain")
            tokenType = nil // Ensure it's nil if not found
        }
           ...
           let savedAccessToken = String(data: accessData, encoding: .utf8)
        {
            ...
            LogManager.logDebug("TokenManager - No accessJwt found in Keychain")
            accessJwt = nil // Ensure it's nil if not found
        }
           ...
           let savedRefreshToken = String(data: refreshData, encoding: .utf8)
        {
            ...
            LogManager.logDebug("TokenManager - No refreshJwt found in Keychain")
            refreshJwt = nil // Ensure it's nil if not found
        }
        LogManager.logDebug("<<< TokenManager.loadInitialTokens FINISHED")
    }
    ...
    public func hasValidTokens() async -> Bool {
        // The session is considered potentially valid if a refresh token exists (in memory or keychain).
        // Access token expiry is handled by the refresh mechanism.
        let refreshTokenExists = await fetchRefreshToken() != nil
        if refreshTokenExists {
            LogManager.logDebug("TokenManager.hasValidTokens - Check passed: Refresh token exists.")
        } else {
            LogManager.logDebug("TokenManager.hasValidTokens - Check failed: No refresh token found.")
        }
    ...
    func shouldRefreshTokens() async -> Bool {
        guard let accessToken = await fetchAccessToken(),
            ...
            return true // Needs refresh if no access token or cannot decode
        }
    ...
    func hasAnyTokens() async -> Bool {
        // Check both in-memory and persisted tokens
        if accessJwt != nil || refreshJwt != nil {
            return true
        }

        let namespace = await getNamespace()
        // Check if we have any tokens in keychain
        if let _ = try? KeychainManager.retrieve(key: "accessJwt", namespace: namespace) {
            return true
        }
        if let _ = try? KeychainManager.retrieve(key: "refreshJwt", namespace: namespace) {
            return true
        }
        return false
    }
    ...
    public func saveTokens(
        ...
        if let accessJwt = accessJwt {
            try await saveAccessToken(accessJwt, type: type)
        }
        if let refreshJwt = refreshJwt {
            try await saveRefreshToken(refreshJwt, type: type)
        }
        try await saveTokenType(type)
    ...
    private func saveAccessToken(_ token: String, type: TokenType) async throws {
    ...
    private func saveRefreshToken(_ token: String, type: TokenType) async throws {
        guard let data = token.data(using: .utf8) else {
            throw TokenError.invalidTokenData
        }
        let namespace = await getNamespace()
    ...
    private func saveTokenType(_ type: TokenType) async throws {
        guard let data = try? JSONEncoder().encode(type) else {
            throw TokenError.invalidTokenData
        }
        try KeychainManager.store(key: "tokenType", value: data, namespace: await getNamespace())
    }

    func fetchAccessToken() async -> String? {
        // Prioritize in-memory token
        if let token = accessJwt {
            // We're not checking expiration here anymore
            LogManager.logDebug("TokenManager - Fetched in-memory accessJwt: \(token.prefix(30))...")
            return token
        }

        // If in-memory is nil, try reading from Keychain directly
        LogManager.logDebug("TokenManager - In-memory access token is nil, attempting Keychain read.")
        let namespace = await getNamespace()
        do {
            let accessData = try KeychainManager.retrieve(key: "accessJwt", namespace: namespace)
            if let savedAccessToken = String(data: accessData, encoding: .utf8) {
                // Cache it in memory and return
                accessJwt = savedAccessToken
                LogManager.logDebug("TokenManager - Fetched accessJwt from Keychain: \(savedAccessToken.prefix(30))...")
                return savedAccessToken
            } else {
                LogManager.logError("TokenManager - Failed to decode accessJwt from Keychain data.")
                return nil
            }
        } catch KeychainError.itemRetrievalError {
            LogManager.logDebug("TokenManager - No accessJwt found in Keychain for namespace \(namespace).")
            return nil
        } catch {
            LogManager.logError("TokenManager - Error retrieving accessJwt from Keychain: \(error)")
            return nil
        }
    }
    ...
    public func fetchRefreshToken() async -> String? {
        ...
        if let token = refreshJwt {
            LogManager.logDebug("TokenManager - Fetched in-memory refreshJwt: \(token.prefix(30))...")
            return token
        }

        // If in-memory is nil, try reading from Keychain directly
        LogManager.logDebug("TokenManager - In-memory refresh token is nil, attempting Keychain read.")
        let namespace = await getNamespace()
        do {
            // Directly attempt to retrieve refresh token without checking tokenType first
            let refreshData = try KeychainManager.retrieve(key: "refreshJwt", namespace: namespace)
            if let savedRefreshToken = String(data: refreshData, encoding: .utf8) {
                // Cache it in memory and return
                refreshJwt = savedRefreshToken
