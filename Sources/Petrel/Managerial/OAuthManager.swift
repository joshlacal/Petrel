...
actor OAuthManager {
  ...
  private func loadInitialDPoPKey() async {
    ...
    do {
      ...
      do {
        ...
        LogManager.logInfo(
          "Generated new DPoP key pair and stored in keychain for namespace: \(namespace)")
      } catch {
    ...
    LogManager.logDebug(
      "OAuth Configuration: clientId=\(oauthConfig.clientId), redirectUri=\(oauthConfig.redirectUri), scopes=\(oauthConfig.scope)"
    )
  ...
  func pushAuthorizationRequest(
    ...
    LogManager.logInfo(
      "Starting PAR request. Current DPoP nonce: \(dpopNonces["bsky.social"] ?? "nil")")
  ...
  func deleteDPoPKey() async {
    do {
      ...
      await EventBus.shared.publish(
        .customEvent(
          name: "DPoPKeyDeleted", data: "DPoP key pair and bindings deleted and nonces cleared"))
    } catch {
  ...
  func handleCallback(url: URL) async throws -> (accessToken: String, refreshToken: String) {
    await EventBus.shared.publish(.oauthCallbackReceived(url)) // Use a more appropriate event

    // --- Ensure DPoP Key is Ready ---
    // Explicitly load/generate the key for the *current* active account
    // *before* proceeding with token exchange. This avoids relying on event timing.
    // Note: This assumes the active account DID has been set correctly *before* this callback is invoked,
    // which seems to be the case based on AuthenticationService flow.
    _ = try await getOrCreateDPoPKeyPair()
    LogManager.logDebug("OAuthManager - Ensured DPoP key pair is ready for handleCallback.")
    // --- End DPoP Key Ensure ---
    ...
    if let httpResponse = response as? HTTPURLResponse {
      ...
      if !(200...299).contains(httpResponse.statusCode) {
        ...
        if let oauthError = try? JSONDecoder().decode(OAuthErrorResponse.self, from: data) {
          throw NSError(
            domain: "OAuthError", code: httpResponse.statusCode,
            userInfo: [
