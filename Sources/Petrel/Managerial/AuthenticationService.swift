//
//  RefactoredAuthenticationService.swift
//  Petrel
//
...
public actor AuthenticationService: AuthServiceProtocol, AuthenticationProvider {
  ...
  private func revokeToken(refreshToken: String, endpoint: String, did: String) async {
    ...
    let parameters: [String: String] = [
      ...
      "client_id": self.oauthConfig.clientId,  // Client ID might be required // FIX: Use self.oauthConfig
    ]
  ...
  public func startOAuthFlow(identifier: String? = nil) async throws -> URL {
    ...
    components.queryItems = [
      ...
      URLQueryItem(name: "client_id", value: self.oauthConfig.clientId),  // FIX: Use self.oauthConfig
      URLQueryItem(name: "redirect_uri", value: self.oauthConfig.redirectUri),  // FIX: Use self.oauthConfig
    ]
  ...
  private func pushAuthorizationRequestForSignUp(
    ...
    var parameters: [String: String] = [
      "client_id": self.oauthConfig.clientId,  // FIX: Use self.oauthConfig
      "redirect_uri": self.oauthConfig.redirectUri,  // FIX: Use self.oauthConfig
      "response_type": "code",
      "code_challenge_method": "S256",
      "code_challenge": codeChallenge,
      "state": state,
      "scope": self.oauthConfig.scope,  // FIX: Use self.oauthConfig // Ensure scope is included
    ]
    ...
    let dpopProof = try await createDPoPProof(
      for: "POST", url: endpoint, type: .authorization, ephemeralKey: ephemeralKey)  // Use passed key
    request.setValue(dpopProof, forHTTPHeaderField: "DPoP")
  ...
  public func refreshTokenIfNeeded() async throws -> Bool {
    ...
    do {
      ...
      guard let savedSession = try await storage.getSession(for: account.did),
            savedSession.refreshToken == tokenResponse.refreshToken else {
      ...
      case .dpopError(let description):
      ...
      case .networkError(let code, let details):
        ...
        throw AuthError.tokenRefreshFailed
      case .decodingError(let underlyingError, let context):
      ...
      case .refreshFunctionError(let error):
          throw error
      }
    } catch {
  ...
  private func performTokenRefreshWithRetry(for did: String, retryCount: Int) async throws -> (Data, HTTPURLResponse) {
    ...
    let requestBody: [String: String] = [
      ...
      "client_id": self.oauthConfig.clientId,
    ]
