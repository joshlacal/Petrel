...
actor ConfigurationManager: ConfigurationManaging {
  ...
  private func loadSettings() async {
    LogManager.logError(
      "CONFIG_LOAD: Starting to load settings from Keychain, current DID='\(self.did ?? "nil")'")
  ...
  func getProtectedResourceMetadata() async -> ProtectedResourceMetadata? {
    LogManager.logError(
      "CONFIG_METADATA: Starting getProtectedResourceMetadata, current DID='\(self.did ?? "nil")'")
  ...
  private func loadSettingsForDID(_ didToLoad: String) async -> Bool {
    ...
    LogManager.logError(
      "CONFIG_LOAD_DID: Finished loading. Actor state: baseURL=\(baseURL), pdsURL=\(pdsURL?.absoluteString ?? "nil"), handle=\(handle ?? "nil"), did=\(self.did ?? "nil")"
    )
  ...
  func getHandle(forAccount did: String) async -> String? {
    if did == self.did, let handle = self.handle {
      return handle
    }
  ...
  func getPDSURL(forAccount did: String) async -> URL? {
    if did == self.did, let pdsURL = self.pdsURL {
      return pdsURL
    }
  ...
  func getProtectedResourceMetadata(forAccount did: String) async -> ProtectedResourceMetadata? {
    if did == self.did, let metadata = self.protectedResourceMetadata {
      return metadata
    }
  ...
  func getAuthorizationServerMetadata(forAccount did: String) async -> AuthorizationServerMetadata?
  {
    if did == self.did, let metadata = self.authorizationServerMetadata {
      return metadata
    }
