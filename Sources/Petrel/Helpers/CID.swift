...
public class DAGCBOR {
    ...
    private static func convertToCBORItem(_ value: Any) throws -> CBOR {
        ...
        case let int as Int:
            if int < 0 {
                return .negativeInt(UInt64(abs(Int64(int) + 1)))
            } else {
        ...
        case let array as [Any]:
            var cborArray = [CBOR]()
            for item in array {
                try cborArray.append(convertToCBORItem(item))
            }
    ...
    public static func decodeCBORItem(_ item: CBOR) throws -> Any {
        ...
        case let .array(array):
            ...
            for item in array {
                try decodedArray.append(decodeCBORItem(item))
            }
            return decodedArray
