...
public struct Blob: Codable, ATProtocolCodable, Hashable, Equatable, Sendable {
  ...
  public func toCBORValue() throws -> Any {
    ...
    map = map.adding(key: "$type", value: self.type)  // Assuming 'type' holds the lexicon identifier like "blob"

    if let refLink = self.ref {
      let refValue = try refLink.toCBORValue()
      map = map.adding(key: "ref", value: refValue)
    }

    map = map.adding(key: "mimeType", value: self.mimeType)
    map = map.adding(key: "size", value: self.size)

    if let cidValue = self.cid {
...
public enum ATIdentifier: ATProtocolValue, CustomStringConvertible, QueryParameterConvertible {
  ...
  public init(from decoder: Decoder) throws {
    ...
    if string.starts(with: "did:") {
      self = .did(try DID(didString: string))
    } else {
      self = .handle(try Handle(handleString: string))
    }
  }
  ...
  public func isEqual(to other: any ATProtocolValue) -> Bool {
    ...
    case (.did(let did1), .did(let did2)):
      return did1 == did2
    case (.handle(let handle1), .handle(let handle2)):
...
public struct TID: ATProtocolValue, CustomStringConvertible, QueryParameterConvertible, Comparable {
  ...
  public init(tidString: String) throws {
    ...
    self.timestamp = (tidValue >> 10) & 0x1F_FFFF_FFFF_FFFF
    self.clockId = tidValue & 0x3FF
    self.originalString = tidString
  }
