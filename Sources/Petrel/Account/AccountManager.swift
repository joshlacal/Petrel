...
actor AccountManager: AccountManaging {
  ...
  private func performStartupRecovery() async {
    ...
    LogManager.logDebug(
      "AccountManager - Performing startup recovery for DID: \(LogManager.logDID(did))")

    // Check if the current DID has a corresponding account
    let account = await getAccount(did: did)
    var hasSession = false
    var hasGatewaySession = false
    ...
    do {
      ...
      LogManager.logDebug(
        "AccountManager - Error checking session during startup recovery: \(error)")
      hasSession = false
    }

    // Check for gateway session
    do {
      let gatewaySession = try await storage.getGatewaySession(for: did)
      hasGatewaySession = gatewaySession != nil
    } catch {
      LogManager.logDebug(
        "AccountManager - Error checking gateway session during startup recovery: \(error)")
    ...
    if account == nil {
      LogManager.logError(
        "AccountManager - Startup recovery detected inconsistent state: currentDID exists but no account found"
      )
      ...
      LogManager.logAuthIncident(
        ...
        details: [
          ...
          "hasGatewaySession": hasGatewaySession,
    ...
    } else if !hasSession, !hasGatewaySession {
      LogManager.logWarning(
        "AccountManager - Startup recovery detected account without session",
    ...
    } else if !hasDPoPKey, hasSession {
      LogManager.logWarning(
        "AccountManager - Startup recovery detected account without DPoP key",
        category: .authentication)
      ...
      LogManager.logDebug(
        "AccountManager - Startup recovery completed: authentication state appears consistent")
  ...
  func setCurrentAccount(did: String) async throws {
    ...
    guard !account.did.isEmpty else {
      LogManager.logError(
        "AccountManager - Cannot set current account, account has invalid DID: \(did)")
    ...
    do {
      ...
      LogManager.logWarning(
        "AccountManager - Failed to check session when setting current account for DID \(LogManager.logDID(did)): \(error)"
      )
    }
    ...
    do {
      ...
      currentDID = did
      LogManager.logInfo(
        "AccountManager - Successfully set current account to DID: \(LogManager.logDID(did))")
  ...
  func getCurrentAccount() async -> Account? {
    ...
    guard let account = await getAccount(did: did) else {
      LogManager.logError(
        "AccountManager - Current DID \(did) exists but account not found in storage")
  ...
  private func clearInconsistentAuthenticationState(for did: String) async {
    LogManager.logError(
      "AccountManager - Clearing inconsistent authentication state for DID: \(LogManager.logDID(did))"
    )
    ...
    do {
      try await storage.deleteSession(for: did)
      LogManager.logDebug(
        "AccountManager - Cleaned up orphaned session for DID: \(LogManager.logDID(did))")
    } catch {
      LogManager.logDebug(
        "AccountManager - No session to clean up for DID: \(LogManager.logDID(did)) or cleanup failed: \(error)"
      )
    }
    ...
    do {
      try await storage.deleteDPoPKey(for: did)
      LogManager.logDebug(
        "AccountManager - Cleaned up orphaned DPoP key for DID: \(LogManager.logDID(did))")
    } catch {
      LogManager.logDebug(
        "AccountManager - No DPoP key to clean up for DID: \(LogManager.logDID(did)) or cleanup failed: \(error)"
      )
    }
    ...
    do {
      ...
      LogManager.logDebug(
        "AccountManager - Cleaned up DPoP nonces for DID: \(LogManager.logDID(did))")
    } catch {
      LogManager.logDebug(
        "AccountManager - Failed to clean up DPoP nonces for DID: \(LogManager.logDID(did)): \(error)"
      )
    }

    LogManager.logInfo(
      "AccountManager - Completed cleanup of inconsistent authentication state for DID: \(LogManager.logDID(did))"
    )
  }
...
enum AccountError: Error, LocalizedError {
  ...
  var recoverySuggestion: String? {
    ...
    case .storageError:
      return
        "Check that you have sufficient storage space and try again. If the problem persists, restart the app."
    case .noActiveAccount:
