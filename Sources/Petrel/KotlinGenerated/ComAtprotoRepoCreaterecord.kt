...
object ComAtprotoRepoCreaterecord {
...
// The handle or DID of the repo (aka, current account).        @SerialName("repo")
    ...
    data class Output(
        ...
        val commit: ComAtprotoRepoDefs.Commitmeta? = null,        @SerialName("validationStatus")
        val validationStatus: String? = null    )

    sealed class Error(val name: String, val description: String?) {
        object Invalidswap: Error("InvalidSwap", "Indicates that 'swapCommit' didn't match current repo commit.")
    }
...
input: ComAtprotoRepoCreaterecord.Input): ATProtoResponse<ComAtprotoRepoCreaterecord.Output> {
