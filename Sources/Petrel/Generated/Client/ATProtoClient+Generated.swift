...
public actor ATProtoClient {
  ...
  public init(
    ...
    LogManager.logInfo(
      "ATProtoClient - Initializing with bskyAppViewDID: \(bskyAppViewDID), bskyChatDID: \(bskyChatDID)"
    )
    ...
    if validationResult.hasIssues {
      ...
      if !validationResult.cleanedOrphanedAccounts.isEmpty {
        LogManager.logWarning(
          "Cleaned up \(validationResult.cleanedOrphanedAccounts.count) orphaned accounts due to race condition. Users may need to re-authenticate."
        )
      }
    } else {
  ...
  private func initializeFromStoredAccount() async {
    LogManager.logInfo(
      "ATProtoClient - initializeFromStoredAccount called, isGatewayMode: \(isGatewayMode)")
    if let account = await accountManager.getCurrentAccount() {
      LogManager.logInfo(
        "ATProtoClient - Found current account: \(account.did), pdsURL: \(account.pdsURL)")
      ...
      LogManager.logInfo(
        "ATProtoClient - Loading service DIDs from stored account: bskyAppViewDID=\(account.bskyAppViewDID), bskyChatDID=\(account.bskyChatDID)"
      )
      ...
      LogManager.logWarning(
        "ATProtoClient - initializeFromStoredAccount: No current account found!")
    }
  }
  ...
  {
    return try await authManager.startOAuthFlowForSignUp(
      pdsURL: pdsURL, bskyAppViewDID: nil, bskyChatDID: nil)
  }
  ...
  public func startOAuthFlow(
    identifier: String? = nil, bskyAppViewDID: String? = nil, bskyChatDID: String? = nil
  ) async throws -> URL {
    return try await authManager.startOAuthFlow(
  ...
  public func getActiveAccountInfo() async -> (did: String?, handle: String?, pdsURL: URL?) {
    ...
    if let account = await accountManager.getCurrentAccount() {
      ...
      LogManager.logWarning(
        "AccountManager returned nil - authentication state may be inconsistent")
  ...
  public func switchToAccount(did: String) async throws {
    ...
    if let account = await accountManager.getAccount(did: did) {
      ...
      if !isGatewayMode {
        ...
        LogManager.logInfo(
          "ATProtoClient - Gateway mode: keeping gateway URL as base URL during account switch")
      }

      // Load and apply service DIDs from account
      LogManager.logInfo(
        "ATProtoClient - Loading service DIDs from account: bskyAppViewDID=\(account.bskyAppViewDID), bskyChatDID=\(account.bskyChatDID)"
      )
      await networkService.setServiceDID(account.bskyAppViewDID, for: "app.bsky")
      await networkService.setServiceDID(account.bskyChatDID, for: "chat.bsky")
    }
  ...
  public func updateServiceDIDs(bskyAppViewDID: String, bskyChatDID: String) async {
    LogManager.logInfo(
      "ATProtoClient - Updating service DIDs: bskyAppViewDID=\(bskyAppViewDID), bskyChatDID=\(bskyChatDID)"
    )
    await networkService.setServiceDID(bskyAppViewDID, for: "app.bsky")
    await networkService.setServiceDID(bskyChatDID, for: "chat.bsky")
  }
  ...
  public func updateAndPersistServiceDIDs(bskyAppViewDID: String, bskyChatDID: String) async throws
  {
    ...
    try await accountManager.updateServiceDIDs(
      bskyAppViewDID: bskyAppViewDID, bskyChatDID: bskyChatDID)
