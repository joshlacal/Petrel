...
public struct BlueCatbirdMlsPromoteAdmin { 

    public static let typeIdentifier = "blue.catbird.mls.promoteAdmin"
...
public enum Error: String, Swift.Error, ATProtoErrorType, CustomStringConvertible {
                ...
                case alreadyAdmin = "AlreadyAdmin"
...
extension ATProtoClient.Blue.Catbird.Mls {
    ...
    public func promoteAdmin(
        ...
        input: BlueCatbirdMlsPromoteAdmin.Input
        
    ) async throws -> (responseCode: Int, data: BlueCatbirdMlsPromoteAdmin.Output?) {
        let endpoint = "blue.catbird.mls.promoteAdmin"
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mls.promoteAdmin")
        ...
        if (200...299).contains(responseCode) {
            do {
                ...
                let decodedData = try decoder.decode(BlueCatbirdMlsPromoteAdmin.Output.self, from: responseData)
                
                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for blue.catbird.mls.promoteAdmin: \(error)")
            ...
            if let atprotoError = ATProtoErrorParser.parse(
                ...
                errorType: BlueCatbirdMlsPromoteAdmin.Error.self
            ) {
                throw atprotoError
            }
