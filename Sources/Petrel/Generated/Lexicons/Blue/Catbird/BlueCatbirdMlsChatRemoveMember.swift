...
public enum BlueCatbirdMlsChatRemoveMember {
    public static let typeIdentifier = "blue.catbird.mlsChat.removeMember"
    ...
    public struct Output: ATProtocolCodable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            epochHint = try container.decodeIfPresent(Int.self, forKey: .epochHint)
        }
...
public extension ATProtoClient.Blue.Catbird.MlsChat {
    ...
    func removeMember(
        input: BlueCatbirdMlsChatRemoveMember.Input

    ) async throws -> (responseCode: Int, data: BlueCatbirdMlsChatRemoveMember.Output?) {
        let endpoint = "blue.catbird.mlsChat.removeMember"
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mlsChat.removeMember")
        ...
        if (200 ... 299).contains(responseCode) {
            do {
                ...
                let decodedData = try decoder.decode(BlueCatbirdMlsChatRemoveMember.Output.self, from: responseData)

                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for blue.catbird.mlsChat.removeMember: \(error)")
