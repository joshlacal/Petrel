...
public enum BlueCatbirdMlsChatValidateWelcome {
    ...
    public struct Input: ATProtocolCodable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            welcomeMessage = try container.decode(Bytes.self, forKey: .welcomeMessage)
        }
    ...
    public struct Output: ATProtocolCodable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            recipientDid = try container.decodeIfPresent(DID.self, forKey: .recipientDid)

            groupId = try container.decodeIfPresent(String.self, forKey: .groupId)

            reserved = try container.decodeIfPresent(Bool.self, forKey: .reserved)

            reservedUntil = try container.decodeIfPresent(ATProtocolDate.self, forKey: .reservedUntil)
        }
...
public extension ATProtoClient.Blue.Catbird.MlsChat {
    ...
    func validateWelcome(
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mlsChat.validateWelcome")
        ...
        if (200 ... 299).contains(responseCode) {
            do {
                ...
                LogManager.logError("Failed to decode successful response for blue.catbird.mlsChat.validateWelcome: \(error)")
