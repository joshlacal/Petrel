...
public struct BlueCatbirdMlsChatRegisterDeviceToken { 

    public static let typeIdentifier = "blue.catbird.mlsChat.registerDeviceToken"
...
extension ATProtoClient.Blue.Catbird.MlsChat {
    ...
    public func registerDeviceToken(
        ...
        input: BlueCatbirdMlsChatRegisterDeviceToken.Input
        
    ) async throws -> (responseCode: Int, data: BlueCatbirdMlsChatRegisterDeviceToken.Output?) {
        let endpoint = "blue.catbird.mlsChat.registerDeviceToken"
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mlsChat.registerDeviceToken")
        ...
        if (200...299).contains(responseCode) {
            do {
                ...
                let decodedData = try decoder.decode(BlueCatbirdMlsChatRegisterDeviceToken.Output.self, from: responseData)
                
                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for blue.catbird.mlsChat.registerDeviceToken: \(error)")
