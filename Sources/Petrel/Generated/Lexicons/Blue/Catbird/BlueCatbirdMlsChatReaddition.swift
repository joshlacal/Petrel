...
public struct BlueCatbirdMlsChatReaddition { 

    public static let typeIdentifier = "blue.catbird.mlsChat.readdition"
...
extension ATProtoClient.Blue.Catbird.MlsChat {
    ...
    public func readdition(
        ...
        input: BlueCatbirdMlsChatReaddition.Input
        
    ) async throws -> (responseCode: Int, data: BlueCatbirdMlsChatReaddition.Output?) {
        let endpoint = "blue.catbird.mlsChat.readdition"
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mlsChat.readdition")
        ...
        if (200...299).contains(responseCode) {
            do {
                ...
                let decodedData = try decoder.decode(BlueCatbirdMlsChatReaddition.Output.self, from: responseData)
                
                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for blue.catbird.mlsChat.readdition: \(error)")
