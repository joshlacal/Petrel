...
public enum BlueCatbirdMlsChatGetAdminStats {
    ...
    public struct ModerationStats: ATProtocolCodable, ATProtocolValue {
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            if pendingReports != other.pendingReports {
                return false
            }
            if resolvedReports != other.resolvedReports {
                return false
            }
            if totalRemovals != other.totalRemovals {
                return false
            }
            if blockConflictsResolved != other.blockConflictsResolved {
                return false
            }
...
public extension ATProtoClient.Blue.Catbird.MlsChat {
    ...
    func getAdminStats(input: BlueCatbirdMlsChatGetAdminStats.Parameters) async throws -> (responseCode: Int, data: BlueCatbirdMlsChatGetAdminStats.Output?) {
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.mlsChat.getAdminStats")
        ...
        if (200 ... 299).contains(responseCode) {
            do {
                ...
                LogManager.logError("Failed to decode successful response for blue.catbird.mlsChat.getAdminStats: \(error)")
