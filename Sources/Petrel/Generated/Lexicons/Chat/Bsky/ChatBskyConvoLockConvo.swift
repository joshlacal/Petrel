...
public struct ChatBskyConvoLockConvo {

    public static let typeIdentifier = "chat.bsky.convo.lockConvo"
...
extension ATProtoClient.Chat.Bsky.Convo {
    ...
    public func lockConvo(

        input: ChatBskyConvoLockConvo.Input

    ) async throws -> (responseCode: Int, data: ChatBskyConvoLockConvo.Output?) {
        let endpoint = "chat.bsky.convo.lockConvo"
        ...
        let serviceDID = await networkService.getServiceDID(for: "chat.bsky.convo.lockConvo")
        ...
        if (200...299).contains(responseCode) {
            ...
            do {
                ...
                let decodedData = try decoder.decode(ChatBskyConvoLockConvo.Output.self, from: responseData)

                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for chat.bsky.convo.lockConvo: \(error)")
