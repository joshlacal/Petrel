...
public struct External: ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            do {
                self.associatedRefs = try container.decodeIfPresent([ComAtprotoRepoStrongRef].self, forKey: .associatedRefs)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
                LogManager.logWarning("Decoding error for optional property 'associatedRefs' — degrading to nil: \(error)")
                self.associatedRefs = nil
            }
        }
...
public struct ViewExternal: ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            do {
                self.readingTime = try container.decodeIfPresent(Int.self, forKey: .readingTime)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
            ...
            do {
                self.source = try container.decodeIfPresent(ViewExternalSource.self, forKey: .source)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
            ...
            do {
                self.associatedProfiles = try container.decodeIfPresent([AppBskyActorDefs.ProfileViewBasic].self, forKey: .associatedProfiles)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
                LogManager.logWarning("Decoding error for optional property 'associatedProfiles' — degrading to nil: \(error)")
                self.associatedProfiles = nil
            }
        }
...
public struct ViewExternalSource: ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            do {
                self.icon = try container.decodeIfPresent(URI.self, forKey: .icon)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
            ...
            do {
                self.theme = try container.decodeIfPresent(ViewExternalSourceTheme.self, forKey: .theme)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
...
public struct ViewExternalSourceTheme: ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            do {
                self.backgroundRGB = try container.decodeIfPresent(ColorRGB.self, forKey: .backgroundRGB)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
                LogManager.logWarning("Decoding error for optional property 'backgroundRGB' — degrading to nil: \(error)")
                self.backgroundRGB = nil
            }
            do {
                self.foregroundRGB = try container.decodeIfPresent(ColorRGB.self, forKey: .foregroundRGB)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
                LogManager.logWarning("Decoding error for optional property 'foregroundRGB' — degrading to nil: \(error)")
                self.foregroundRGB = nil
            }
            do {
                self.accentRGB = try container.decodeIfPresent(ColorRGB.self, forKey: .accentRGB)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
            ...
            do {
                self.accentForegroundRGB = try container.decodeIfPresent(ColorRGB.self, forKey: .accentForegroundRGB)
            } catch {
                // Forward compatibility: a malformed or unknown-shaped optional field
                // must not fail the whole response.
                LogManager.logWarning("Decoding error for optional property 'accentForegroundRGB' — degrading to nil: \(error)")
                self.accentForegroundRGB = nil
            }
        }
