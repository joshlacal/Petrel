...
public enum ChatBskyConvoGetLog {
    ...
    public struct Output: ATProtocolCodable {
        ...
        public func toCBORValue() throws -> Any {
            ...
            let logsValue = try (logs as? DAGCBOREncodable)?.toCBORValue() ?? logs
    ...
    public enum OutputLogsUnion: Codable, ATProtocolCodable, ATProtocolValue, Sendable, Equatable {
        ...
        public mutating func loadPendingData() async {
            ...
            case var .chatBskyConvoDefsLogLeaveConvo(value):
                ...
                // Update the enum case with the potentially updated value
                self = .chatBskyConvoDefsLogLeaveConvo(value)
            case var .chatBskyConvoDefsLogMuteConvo(value):
                // Since ATProtocolValue already includes PendingDataLoadable,
                // we can directly call loadPendingData without conditional casting
                await value.loadPendingData()
                // Update the enum case with the potentially updated value
                self = .chatBskyConvoDefsLogMuteConvo(value)
            case var .chatBskyConvoDefsLogUnmuteConvo(value):
                // Since ATProtocolValue already includes PendingDataLoadable,
                // we can directly call loadPendingData without conditional casting
                await value.loadPendingData()
                // Update the enum case with the potentially updated value
            ...
            case var .chatBskyConvoDefsLogDeleteMessage(value):
                ...
                // Update the enum case with the potentially updated value
                self = .chatBskyConvoDefsLogDeleteMessage(value)
            case var .chatBskyConvoDefsLogReadMessage(value):
                // Since ATProtocolValue already includes PendingDataLoadable,
                // we can directly call loadPendingData without conditional casting
                await value.loadPendingData()
                // Update the enum case with the potentially updated value
                self = .chatBskyConvoDefsLogReadMessage(value)
            case var .chatBskyConvoDefsLogAddReaction(value):
                // Since ATProtocolValue already includes PendingDataLoadable,
                // we can directly call loadPendingData without conditional casting
                await value.loadPendingData()
                // Update the enum case with the potentially updated value
                self = .chatBskyConvoDefsLogAddReaction(value)
            case var .chatBskyConvoDefsLogRemoveReaction(value):
                // Since ATProtocolValue already includes PendingDataLoadable,
                // we can directly call loadPendingData without conditional casting
                await value.loadPendingData()
                // Update the enum case with the potentially updated value
