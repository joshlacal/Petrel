...
public actor ATProtoClient {
    ...
    /// This method uses the com.atproto.server.createSession endpoint for authentication
    /// and should be used with app passwords for enhanced security.
    /// - Parameters:
    ///   - identifier: The user identifier (handle or email).
    ///   - password: The user's app password (not their main account password).
    ///   - bskyAppViewDID: Optional custom AppView DID to use for this account
    ///   - bskyChatDID: Optional custom Chat DID to use for this account
    public func loginWithPassword(
        ...
        // Get account info from auth service
        let accountInfo = try await authService.loginWithPassword(
        ...
        // Store it temporarily for immediate access
        justAuthenticatedAccount = accountInfo
        LogManager.logDebug("Stored temporary account info after password login: \(accountInfo.did)")
