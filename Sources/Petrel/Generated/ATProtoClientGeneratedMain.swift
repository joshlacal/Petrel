...
public actor ATProtoClient: AuthenticationDelegate, DIDResolving {
  ...
  public func logout() async throws {
    ...
    LogManager.logError(
      "LOGOUT_DEBUG: Value of self.did right before guard: \(String(describing: self.did))")

    // Get the current DID either from memory or from lastActiveDID in Keychain
    var effectiveDID = self.did
    if effectiveDID == nil {
      LogManager.logError(
  ...
  public func switchToAccount(did: String) async throws -> Bool {
    ...
    if let pdsURL = pdsURLFromConfig {
      ...
      self.baseURL = URL(string: "https://bsky.social")!
      self.pdsURL = nil
    }
    ...
    self.handle = await configManager.getHandle()
    LogManager.logError(
      "SWITCH_ACC: ATProtoClient internal state updated: did=\(self.did ?? "nil"), handle=\(self.handle ?? "nil")"
    )
...
public final class Tools: @unchecked Sendable {
    ...
    public final class Ozone: @unchecked Sendable {
        ...
        public lazy var signature: Signature = {
            return Signature(networkManager: self.networkManager)
        }()
        ...
        public lazy var set: Set = {
            return Set(networkManager: self.networkManager)
        }()
        ...
        public lazy var setting: Setting = {
            return Setting(networkManager: self.networkManager)
        }()
...
public final class Com: @unchecked Sendable {
    ...
    public final class Atproto: @unchecked Sendable {
        ...
        public lazy var lexicon: Lexicon = {
            return Lexicon(networkManager: self.networkManager)
        }()
