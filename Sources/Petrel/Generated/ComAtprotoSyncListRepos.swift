...
public enum ComAtprotoSyncListRepos {
    ...
    public struct Repo: ATProtocolCodable, ATProtocolValue {
        ...
        public var hasPendingData: Bool {
            ...
            if !hasPending, let loadable = head as? PendingDataLoadable {
                hasPending = loadable.hasPendingData
            }
            ...
            if !hasPending, let value = active, let loadable = value as? PendingDataLoadable {
                hasPending = loadable.hasPendingData
            }
        ...
        public mutating func loadPendingData() async {
            ...
            if let loadable = head as? PendingDataLoadable, loadable.hasPendingData {
                var mutableValue = loadable
                await mutableValue.loadPendingData()
                // Only update if we can safely cast back to the expected type
                if let updatedValue = mutableValue as? String {
            ...
            if let value = active, var loadableValue = value as? PendingDataLoadable, loadableValue.hasPendingData {
