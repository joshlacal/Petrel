...
public struct AppBskyRichtextFacet: ATProtocolCodable, ATProtocolValue {
    ...
    public enum AppBskyRichtextFacetFeaturesUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            LogManager.logDebug("AppBskyRichtextFacetFeaturesUnion decoding: \(typeValue)")

            switch typeValue {
            case "app.bsky.richtext.facet#mention":
                LogManager.logDebug("Decoding as app.bsky.richtext.facet#mention")
                let value = try AppBskyRichtextFacet.Mention(from: decoder)
                self = .appBskyRichtextFacetMention(value)
            case "app.bsky.richtext.facet#link":
                LogManager.logDebug("Decoding as app.bsky.richtext.facet#link")
                let value = try AppBskyRichtextFacet.Link(from: decoder)
                self = .appBskyRichtextFacetLink(value)
            case "app.bsky.richtext.facet#tag":
                LogManager.logDebug("Decoding as app.bsky.richtext.facet#tag")
                let value = try AppBskyRichtextFacet.Tag(from: decoder)
                self = .appBskyRichtextFacetTag(value)
            default:
                LogManager.logDebug("AppBskyRichtextFacetFeaturesUnion decoding encountered an unexpected type: \(typeValue)")
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            case let (.appBskyRichtextFacetLink(selfValue),
                ...
                return selfValue == otherValue
            case let (.appBskyRichtextFacetTag(selfValue),
    ...
    public enum FeaturesForUnionArray: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            switch (self, otherValue) {
            case let (.mention(selfValue),
                      .mention(otherValue)):
                return selfValue == otherValue
            case let (.link(selfValue),
                      .link(otherValue)):
                return selfValue == otherValue
            case let (.tag(selfValue),
                      .tag(otherValue)):
                return selfValue == otherValue
            case let (.unexpected(selfValue), .unexpected(otherValue)):
                return selfValue.isEqual(to: otherValue)
