...
public indirect enum ATProtocolValueContainer: ATProtocolCodable, ATProtocolValue {
    ...
    private static func isSpecTolerantContainerMatch(
        ...
        case let (.link(a), .string(b)):
            return a.cid.string == b

        case let (.string(a), .link(b)):
            return a == b.cid.string

        case let (.bytes(a), .string(b)):
            return a.data == Data(base64Encoded: b)

        case let (.string(a), .bytes(b)):
            return Data(base64Encoded: a) == b.data

        case let (.number(a), .string(b)):
            return String(a) == b

        case let (.string(a), .number(b)):
            return a == String(b)
    ...
    public static func fromCBOR(
        ...
        case let .map(dict):
            ...
            if decodedDict.count == 1 {
                if let cidStringContainer = decodedDict["$link"], case let .string(cidString) = cidStringContainer {
                    return try .link(ATProtoLink(cidString: cidString))
                }
                if let base64Container = decodedDict["$bytes"], case let .string(base64String) = base64Container {
