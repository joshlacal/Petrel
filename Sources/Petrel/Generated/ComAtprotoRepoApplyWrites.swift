...
public struct ComAtprotoRepoApplyWrites {
    ...
    public enum InputWritesUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            LogManager.logDebug("InputWritesUnion decoding: \(typeValue)")

            switch typeValue {
            case "com.atproto.repo.applyWrites#create":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#create")
                let value = try ComAtprotoRepoApplyWrites.Create(from: decoder)
                self = .comAtprotoRepoApplyWritesCreate(value)
            case "com.atproto.repo.applyWrites#update":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#update")
                let value = try ComAtprotoRepoApplyWrites.Update(from: decoder)
                self = .comAtprotoRepoApplyWritesUpdate(value)
            case "com.atproto.repo.applyWrites#delete":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#delete")
                let value = try ComAtprotoRepoApplyWrites.Delete(from: decoder)
                self = .comAtprotoRepoApplyWritesDelete(value)
            default:
                LogManager.logDebug("InputWritesUnion decoding encountered an unexpected type: \(typeValue)")
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            case let (.comAtprotoRepoApplyWritesCreate(selfValue),
                      .comAtprotoRepoApplyWritesCreate(otherValue)):
                return selfValue == otherValue
            case let (.comAtprotoRepoApplyWritesUpdate(selfValue),
                      .comAtprotoRepoApplyWritesUpdate(otherValue)):
                return selfValue == otherValue
            case let (.comAtprotoRepoApplyWritesDelete(selfValue),
    ...
    public enum OutputResultsUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            let typeValue = try container.decode(String.self, forKey: .type)
            LogManager.logDebug("OutputResultsUnion decoding: \(typeValue)")

            switch typeValue {
            case "com.atproto.repo.applyWrites#createResult":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#createResult")
            ...
            case "com.atproto.repo.applyWrites#updateResult":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#updateResult")
            ...
            case "com.atproto.repo.applyWrites#deleteResult":
                LogManager.logDebug("Decoding as com.atproto.repo.applyWrites#deleteResult")
                let value = try ComAtprotoRepoApplyWrites.DeleteResult(from: decoder)
                self = .comAtprotoRepoApplyWritesDeleteResult(value)
            default:
                LogManager.logDebug("OutputResultsUnion decoding encountered an unexpected type: \(typeValue)")
                let unknownValue = try ATProtocolValueContainer(from: decoder)
                self = .unexpected(unknownValue)
            }
        }
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            case let .comAtprotoRepoApplyWritesCreateResult(value):
                LogManager.logDebug("Encoding com.atproto.repo.applyWrites#createResult")
            ...
            case let .comAtprotoRepoApplyWritesUpdateResult(value):
                LogManager.logDebug("Encoding com.atproto.repo.applyWrites#updateResult")
            ...
            case let .comAtprotoRepoApplyWritesDeleteResult(value):
                LogManager.logDebug("Encoding com.atproto.repo.applyWrites#deleteResult")
                try container.encode("com.atproto.repo.applyWrites#deleteResult", forKey: .type)
                try value.encode(to: encoder)
            case let .unexpected(ATProtocolValueContainer):
                LogManager.logDebug("OutputResultsUnion encoding unexpected value")
                try ATProtocolValueContainer.encode(to: encoder)
            }
        }
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            switch (self, otherValue) {
            case let (.comAtprotoRepoApplyWritesCreateResult(selfValue),
                      .comAtprotoRepoApplyWritesCreateResult(otherValue)):
                return selfValue == otherValue
            case let (.comAtprotoRepoApplyWritesUpdateResult(selfValue),
                      .comAtprotoRepoApplyWritesUpdateResult(otherValue)):
                return selfValue == otherValue
            case let (.comAtprotoRepoApplyWritesDeleteResult(selfValue),
    ...
    public enum ComAtprotoRepoApplyWritesWritesUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            let typeValue = try container.decode(String.self, forKey: .type)
            LogManager.logDebug("ComAtprotoRepoApplyWritesWritesUnion decoding: \(typeValue)")
            ...
            case "com.atproto.repo.applyWrites#delete":
                ...
                self = .comAtprotoRepoApplyWritesDelete(value)
            default:
                LogManager.logDebug("ComAtprotoRepoApplyWritesWritesUnion decoding encountered an unexpected type: \(typeValue)")
                let unknownValue = try ATProtocolValueContainer(from: decoder)
                self = .unexpected(unknownValue)
            }
        }
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            case let .comAtprotoRepoApplyWritesDelete(value):
                ...
                try value.encode(to: encoder)
            case let .unexpected(ATProtocolValueContainer):
                LogManager.logDebug("ComAtprotoRepoApplyWritesWritesUnion encoding unexpected value")
                try ATProtocolValueContainer.encode(to: encoder)
            }
        }
...
public extension ATProtoClient.Com.Atproto.Repo {
    ...
    func applyWrites(
        ...
        let decodedData = try? ZippyJSONDecoder().decode(ComAtprotoRepoApplyWrites.Output.self, from: responseData)
