...
public struct AppBskyEmbedRecordWithMedia: ATProtocolCodable, ATProtocolValue {
    ...
    public enum ViewMediaUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            ...
            LogManager.logDebug("ViewMediaUnion decoding: \(typeValue)")
            ...
            default:
                LogManager.logDebug("ViewMediaUnion decoding encountered an unexpected type: \(typeValue)")
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            case let .unexpected(ATProtocolValueContainer):
                LogManager.logDebug("ViewMediaUnion encoding unexpected value")
    ...
    public enum AppBskyEmbedRecordWithMediaMediaUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            let typeValue = try container.decode(String.self, forKey: .type)
            LogManager.logDebug("AppBskyEmbedRecordWithMediaMediaUnion decoding: \(typeValue)")

            switch typeValue {
            case "app.bsky.embed.images":
                LogManager.logDebug("Decoding as app.bsky.embed.images")
                let value = try AppBskyEmbedImages(from: decoder)
                self = .appBskyEmbedImages(value)
            case "app.bsky.embed.video":
                LogManager.logDebug("Decoding as app.bsky.embed.video")
                let value = try AppBskyEmbedVideo(from: decoder)
                self = .appBskyEmbedVideo(value)
            case "app.bsky.embed.external":
                LogManager.logDebug("Decoding as app.bsky.embed.external")
                let value = try AppBskyEmbedExternal(from: decoder)
                self = .appBskyEmbedExternal(value)
            default:
                LogManager.logDebug("AppBskyEmbedRecordWithMediaMediaUnion decoding encountered an unexpected type: \(typeValue)")
                let unknownValue = try ATProtocolValueContainer(from: decoder)
                self = .unexpected(unknownValue)
            }
        }
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            case let .appBskyEmbedImages(value):
                LogManager.logDebug("Encoding app.bsky.embed.images")
                try container.encode("app.bsky.embed.images", forKey: .type)
                try value.encode(to: encoder)
            case let .appBskyEmbedVideo(value):
                LogManager.logDebug("Encoding app.bsky.embed.video")
                try container.encode("app.bsky.embed.video", forKey: .type)
                try value.encode(to: encoder)
            case let .appBskyEmbedExternal(value):
                LogManager.logDebug("Encoding app.bsky.embed.external")
                try container.encode("app.bsky.embed.external", forKey: .type)
                try value.encode(to: encoder)
            case let .unexpected(ATProtocolValueContainer):
                LogManager.logDebug("AppBskyEmbedRecordWithMediaMediaUnion encoding unexpected value")
                try ATProtocolValueContainer.encode(to: encoder)
            }
        }
        ...
        public func isEqual(to other: any ATProtocolValue) -> Bool {
            ...
            switch (self, otherValue) {
            case let (.appBskyEmbedImages(selfValue),
                      .appBskyEmbedImages(otherValue)):
                return selfValue == otherValue
            case let (.appBskyEmbedVideo(selfValue),
                      .appBskyEmbedVideo(otherValue)):
                return selfValue == otherValue
            case let (.appBskyEmbedExternal(selfValue),
