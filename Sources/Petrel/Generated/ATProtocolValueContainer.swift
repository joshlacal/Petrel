public indirect enum ATProtocolValueContainer: ATProtocolCodable, ATProtocolValue {
    ...
    public init(from decoder: Decoder) throws {
        ...
        {
            LogManager.logDebug("Attempting to decode type: \(typeValue)")
    ...
    private static func decodeSingleValue(from decoder: Decoder) throws -> ATProtocolValueContainer {
        ...
        LogManager.logDebug("Attempting to decode as String")
        if let stringValue = try? container.decode(String.self) {
            return .string(stringValue)
        }
        LogManager.logDebug("Failed to decode as String, attempting to decode as Int")
        if let intValue = try? container.decode(Int.self) {
            return .number(intValue)
        }
        LogManager.logDebug("Failed to decode as Int, attempting to decode as BigNumber")
        if let bigNumberString = try? container.decode(String.self) {
            return .bigNumber(bigNumberString)
        }
        LogManager.logDebug("Failed to decode as BigNumber, attempting to decode as Bool")
        if let boolValue = try? container.decode(Bool.self) {
            return .bool(boolValue)
        }
        LogManager.logDebug("Failed to decode as Bool, attempting to decode as Null")
        if container.decodeNil() {
            return .null
        }

        LogManager.logDebug("Failed to decode as basic type, attempting to decode as Array or Object")
    ...
    private static func decodeAny(from container: KeyedDecodingContainer<DynamicCodingKeys>) throws -> ATProtocolValueContainer {
        ...
        for key in container.allKeys {
            LogManager.logDebug("Decoding key: \(key.stringValue)")
            if let boolValue = try? container.decode(Bool.self, forKey: key) {
                ...
                while !arrayContainer.isAtEnd {
                    LogManager.logDebug("Decoding array element at index \(arrayContainer.currentIndex)")
    ...
    private static func decodeAny(from unkeyedContainer: inout UnkeyedDecodingContainer) throws -> [ATProtocolValueContainer] {
        ...
        while !unkeyedContainer.isAtEnd {
            LogManager.logDebug("Decoding array element at index \(unkeyedContainer.currentIndex)")
