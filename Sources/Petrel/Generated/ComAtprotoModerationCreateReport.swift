...
public struct ComAtprotoModerationCreateReport {
    ...
    public enum ComAtprotoModerationCreateReportSubjectUnion: Codable, ATProtocolCodable, ATProtocolValue {
        ...
        public init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            let typeValue = try container.decode(String.self, forKey: .type)
            LogManager.logDebug("ComAtprotoModerationCreateReportSubjectUnion decoding: \(typeValue)")
            ...
            case "com.atproto.repo.strongRef":
                ...
                self = .comAtprotoRepoStrongRef(value)
            default:
                LogManager.logDebug("ComAtprotoModerationCreateReportSubjectUnion decoding encountered an unexpected type: \(typeValue)")
                let unknownValue = try ATProtocolValueContainer(from: decoder)
                self = .unexpected(unknownValue)
            }
        }
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            case let .comAtprotoRepoStrongRef(value):
                ...
                try value.encode(to: encoder)
            case let .unexpected(ATProtocolValueContainer):
                LogManager.logDebug("ComAtprotoModerationCreateReportSubjectUnion encoding unexpected value")
                try ATProtocolValueContainer.encode(to: encoder)
            }
        }
...
public extension ATProtoClient.Com.Atproto.Moderation {
    ...
    func createReport(
        ...
        let decodedData = try? ZippyJSONDecoder().decode(ComAtprotoModerationCreateReport.Output.self, from: responseData)
