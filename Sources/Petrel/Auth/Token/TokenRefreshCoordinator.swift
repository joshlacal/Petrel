                        ...
                        if (200 ..< 300).contains(response.statusCode) {
                            ...
                            if let legacyResponse = try? decoder.decode(LegacyRefreshResponse.self, from: data) {
                                ...
                                let result = RefreshResult(
                                    ...
                                    expiresIn: 3600, // Default 1 hour for legacy tokens
                                    refreshedAt: Date(),
                                    originalResponse: (data, response)
                                )
                                await self.updateLastResult(result)
                            ...
                            LogManager.logError(
                                "TokenRefreshCoordinator: Failed to decode 2xx response as OAuth or legacy format. Preview: \(responsePreview)",
    ...
    private func createErrorFromResult(data: Data, response: HTTPURLResponse) -> RefreshError {
        ...
        if let legacyError = try? JSONDecoder().decode(LegacyATProtoError.self, from: data) {
            ...
            case "authenticationrequired", "invalidtoken", "expiredsession":
                // These indicate the refresh token is no longer valid
                LogManager.logAuthIncident(
                ...
                return .invalidGrant("Legacy auth error: \(legacyError.error) - \(legacyError.message ?? "")")
            default:
...
// MARK: - Legacy AT Proto Refresh Response

/// Response format from com.atproto.server.refreshSession (legacy bearer tokens)
/// Uses camelCase keys (accessJwt, refreshJwt) instead of OAuth snake_case (access_token, refresh_token)
private struct LegacyRefreshResponse: Decodable {
    let accessJwt: String
    let refreshJwt: String
    // Optional fields from the legacy response
    let handle: String?
    let did: String?
    let active: Bool?
}

// MARK: - Legacy AT Proto Error Response

/// Error response format from AT Proto endpoints (legacy format)
/// Uses "error" and "message" keys instead of OAuth's "error" and "error_description"
private struct LegacyATProtoError: Decodable {
    let error: String
    let message: String?
}
