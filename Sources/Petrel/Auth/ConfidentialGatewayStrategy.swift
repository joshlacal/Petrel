...
private let logger = Logger(subsystem: "blue.catbird.Petrel", category: "ConfidentialGatewayStrategy")
...
actor ConfidentialGatewayStrategy: AuthStrategy {
    ...
    func handleOAuthCallback(url: URL) async throws -> (did: String, handle: String?, pdsURL: URL) {
        ...
        LogManager.logInfo("ConfidentialGatewayStrategy - Saving account for DID: \(sessionInfo.did), pdsURL: \(gatewayURL)")
        ...
        LogManager.logInfo("ConfidentialGatewayStrategy - Setting current account to DID: \(sessionInfo.did)")
        ...
        LogManager.logInfo("ConfidentialGatewayStrategy - Account setup complete for DID: \(sessionInfo.did)")
    ...
    func logout() async throws {
        ...
        if let session = try? await storage.getGatewaySession(for: did) {
            ...
            logger.info("🚪 Calling gateway logout for DID \(did.prefix(20))...: \(logoutURL.absoluteString)")
            ...
            do {
                ...
                logger.warning("🚪 Gateway logout request failed (continuing anyway): \(error.localizedDescription)")
            }
        } else {
            logger.info("🚪 No gateway session found for DID \(did.prefix(20))..., skipping gateway logout call")
        }
    ...
    func prepareAuthenticatedRequest(_ request: URLRequest) async throws -> URLRequest {
        ...
        LogManager.logInfo("ConfidentialGatewayStrategy - Adding Bearer token to request: \(request.url?.absoluteString ?? "unknown")")
        request.setValue("Bearer \(session)", forHTTPHeaderField: "Authorization")
        return request
    }
    ...
    func refreshTokenIfNeeded() async throws -> TokenRefreshResult {
        ...
        guard (try? await storage.getGatewaySession(for: currentAccount.did)) != nil else {
            logger.warning("refreshTokenIfNeeded: No gateway session found for DID: \(currentAccount.did.prefix(20))...")
            throw GatewayError.missingSession
        }
        return .stillValid
    }
    ...
    func handleUnauthorizedResponse(
        ...
        // For non-gateway 401s, just throw auth required without clearing session
        logger.info("Received 401 from non-gateway service: \(request.url?.absoluteString ?? "unknown")")
        throw GatewayError.sessionExpired
    }
    ...
    private func gatewaySession() async throws -> String {
        ...
        guard let session = try await storage.getGatewaySession(for: currentAccount.did) else {
            LogManager.logError("ConfidentialGatewayStrategy - No gateway session found for DID: \(currentAccount.did.prefix(20))...")
            throw GatewayError.missingSession
        }
        LogManager.logDebug("ConfidentialGatewayStrategy - Found gateway session for \(currentAccount.did.prefix(20))...: \(session.prefix(8))...")
        return session
    }
